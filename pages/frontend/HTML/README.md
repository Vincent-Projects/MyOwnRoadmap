# HTML

In this section i'll put everything i know about HTML5, and i'll try to gather all the useful resources i used and those that i use when i don't know something.

Here's what will be covered :

<details><summary>Tags</summary>

-   !DOCTYPE
-   html

<details><summary>head</summary>

| tags   | description | details |
| ------ | ----------- | ------- |
| meta   |             |         |
| title  |             |         |
| style  |             |         |
| script |             |         |

</details>

<details><summary>body</summary>

| tags       | description | details |
| ---------- | ----------- | ------- |
| h1 - h6    |             |         |
| p          |             |         |
| div        |             |         |
| header     |             |         |
| footer     |             |         |
| article    |             |         |
| strong     |             |         |
| em         |             |         |
| ol         |             |         |
| ul         |             |         |
| li         |             |         |
| br         |             |         |
| hr         |             |         |
| img        |             |         |
| section    |             |         |
| blockquote |             |         |
| a          |             |         |
| form       |             |         |
| input      |             |         |
| label      |             |         |
| button     |             |         |
| textarea   |             |         |
| table      |             |         |
| figure     |             |         |
| figcaption |             |         |
| main       |             |         |
| noscript   |             |         |
| select     |             |         |
| option     |             |         |
| span       |             |         |

</details>

</details>
<details><summary>Attribute</summary>

Each element has its own attribute and some are common to all. I'll put the attribute of an element in details of its tags above. Here are some for reference.

| Attribute | Description | Details |
| --------- | ----------- | ------- |
| class     |             |         |
| id        |             |         |
| href      |             |         |
| src       |             |         |
| alt       |             |         |
| style     |             |         |
| data-\*   |             |         |
| type      |             |         |
| name      |             |         |
| value     |             |         |

</details>

---

### Resources

Here are the resouces that will explain this better.

-   [W3C](https://www.w3schools.com/html/default.asp)
-   [MDN](https://developer.mozilla.org/en-US/docs/Web/HTML/Element)
-   [Learn HTML - Javatpoint](https://www.javatpoint.com/html-tutorial)
-   [TutorialPoint](https://www.tutorialspoint.com/html/index.htm)
-   [HTML Standard - Whatwg](https://html.spec.whatwg.org/)
